...
document.addEventListener('DOMContentLoaded', () => {
  fetch('/api/events')
    ...
    .then(result => {
      ...
      allEvents = result.rows.slice(0, 5);
      initCarousel();
    })
    .catch(err => {
...
function showCarouselPlaceholder() {
  ...
  if (!track) return;
  track.innerHTML = `
    <div class="event-slide" style="display:flex; align-items:center; 
         justify-content:center; width:100%; min-height:220px;">
      <div style="text-align:center; color: #ccc;">
        <p style="margin-top: 12px; font-size: 1rem;">No events to display at the moment.</p>
      </div>
    </div>
  `;
}
